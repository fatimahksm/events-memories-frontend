...
export function UploadPanel({dictionary,eventSlug,onClose,onUploaded,onAllSucceeded}:{dictionary:Dictionary;eventSlug:string;onClose:()=>void;onUploaded:()=>void;onAllSucceeded?:()=>void}){
 const inputRef=useRef<HTMLInputElement>(null); const [guestName,setGuestName]=useState(''); const [visibility,setVisibility]=useState<MediaVisibility>('PUBLIC'); const [files,setFiles]=useState<SelectedUploadFile[]>([]); const [formError,setFormError]=useState<string>(); const running=useRef(new Map<string,XMLHttpRequest>());
 ...
 async function uploadOne(item:SelectedUploadFile):Promise<boolean>{patch(item.id,{state:'requesting',error:undefined});try{
   ...
   if(rejected){patch(item.id,{state:'error',error:dictionary.upload.rejected});return false;}
   patch(item.id,{state:'success',progress:100});
   return true;
 }catch(err){running.current.delete(item.id);patch(item.id,{state:'error',error:errorMessage(err,dictionary.upload.genericError)});return false;}}
 ...
 async function uploadAll(){
   const queue=files.filter(f=>(f.state==='idle'||f.state==='error')&&!f.error);
   const results=await runPool(queue,Math.max(1,appConfig.uploads.maxConcurrentUploads),uploadOne);
   onUploaded();
...
async function runPool<T>(items:T[],concurrency:number,worker:(item:T)=>Promise<boolean>):Promise<boolean[]>{const results:boolean[]=new Array(items.length);let next=0;async function consume(){while(true){const i=next++;if(i>=items.length)return;results[i]=await worker(items[i]);}}await Promise.all(Array.from({length:Math.min(concurrency,items.length)},consume));return results;}
