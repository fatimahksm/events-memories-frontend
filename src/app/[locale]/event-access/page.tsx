...
export default function EventAccessPage() {
  return <Suspense fallback={null}><EventAccessResolver /></Suspense>;
}

function EventAccessResolver() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    apiFetch<Me>('/auth/event-access', { method: 'POST', body: JSON.stringify({ token }) })
