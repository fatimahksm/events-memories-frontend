...
test('creates and publishes an event with a public link, owner link, and QR code', async ({ page }) => {
  await loginAsAdmin(page);
  await createOwner(page, 'Wizard Test Owner');
  const event = await createEvent(page, `Wizard Test Event ${Date.now()}`);

  expect(event.accessToken).toBeTruthy();
  expect(event.slug).toBeTruthy();

  await expect(page.locator('text=Public guest link')).toBeVisible();
  await expect(page.locator('text=Owner access link (this event only)')).toBeVisible();
